...
data CardanoDbSyncException = CardanoDbSyncException String
  deriving stock (Show)
...
dbSyncSlotNumber (Conn pool) = Pool.withResource pool $ \conn -> do
    ...
    case res of
        [PQ.Only n] -> return $ unsafeSlotFromPlutus $ Plutus.Slot n
...
dbSyncQueryUtxo conn = GYQueryUTxO
    { gyQueryUtxosAtAddress'   = dbSyncQueryUtxosAtAddress conn
    , gyQueryUtxosAtTxOutRefs' = dbSyncQueryUtxosAtTxOutRefs conn
    , gyQueryUtxoAtTxOutRef'   = dbSyncQueryUtxoAtTxOutRef conn
    }

-- SELECT tx_id, tx.hash, index, address, value, data_hash FROM tx_out, tx WHERE tx_out.tx_id = tx.id AND address = 'addr_test1vrjt027trymct9tx8zmj4sk9hhdfsmhz7tl3u3df928rz3sfpaxk0';
...
dbSyncQueryUtxosAtAddress (Conn pool) addr = Pool.withResource pool $ \conn -> do
    res <- PQ.query conn
        "SELECT tx.hash, index, address, value :: bigint, to_json(array(select json_build_object('policy', encode(policy, 'hex'), 'name', encode(name, 'hex'), 'quantity', quantity) from ma_tx_out, multi_asset where multi_asset.id = ma_tx_out.ident AND tx_out_id = tx_out.id)) as assets, data_hash FROM tx_out, tx WHERE tx_out.tx_id = tx.id AND NOT EXISTS (SELECT FROM tx_in WHERE tx_out_id = tx.id AND tx_out_index = index) AND address = ?;"
    ...
    let utxos :: [GYUTxO]
        utxos =
            ...
            | (RawBytes txid, txix, addr32, lovelace, PQ.Aeson (MA value), mdh) <- res
            ]
...
dbSyncGetSystemStart (Conn pool) = Pool.withResource pool $ \conn -> do
    ...
    case res of
        ...
        _              -> throwIO $ CardanoDbSyncException "dbSyncGetSystemStart: zero or multipl SQL results"

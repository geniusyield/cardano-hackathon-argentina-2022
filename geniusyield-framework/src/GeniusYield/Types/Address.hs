...
-- >>> import qualified Web.HttpApiData            as Web
--
-- >>> let addr = addressFromTextUnsafe "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5"
...
instance Show GYAddress where
    showsPrec d addr = showParen (d > 10) $
        showString "addressFromTextUnsafe " .
...
addressToApi' :: GYAddress -> Api.AddressInEra Api.AlonzoEra
addressToApi' = coerce addrAnyToAlonzoEra

-- not exported
addrAnyToAlonzoEra :: Api.AddressAny -> Api.AddressInEra Api.AlonzoEra
addrAnyToAlonzoEra (Api.AddressByron   addr) = Api.AddressInEra Api.ByronAddressInAnyEra                            addr
addrAnyToAlonzoEra (Api.AddressShelley addr) = Api.AddressInEra (Api.ShelleyAddressInEra Api.ShelleyBasedEraAlonzo) addr
...
--
addressFromPlutus :: GYNetworkId -> Plutus.Address -> Maybe GYAddress
addressFromPlutus nid =
    either (const Nothing) (Just . addressFromApi . f) . Conv.toCardanoAddressInEra (toApiNetworkId nid)
  where
    f :: Api.S.AddressInEra era -> Api.S.AddressAny
    f (Api.S.AddressInEra _ addr) = Api.toAddressAny addr
...
-- /note:/ no stake credential.
--
addressFromValidator :: GYNetworkId -> GYValidator -> GYAddress
...
addressFromTextMaybe :: T.Text -> Maybe GYAddress
addressFromTextMaybe = coerce (Api.deserialiseAddress Api.AsAddressAny)

addressFromTextUnsafe :: T.Text -> GYAddress
addressFromTextUnsafe t = fromMaybe
    (error $ "Not an address: " ++ show t)
    (addressFromTextMaybe t)

addressToText :: GYAddress -> T.Text
...
instance Printf.PrintfArg GYAddress where
    formatArg addr ff = Printf.formatArg (addressToText addr) ff
...
-- Just (addressFromTextUnsafe "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5")
--
...
-- Right (addressFromTextUnsafe "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5")
--
...
instance Web.FromHttpApiData GYAddress where
    parseUrlPiece t = case Api.deserialiseFromRawBytesHex Api.AsAddressAny (TE.encodeUtf8 t) of
        ...
        Left x     -> Left $ "Not an address: " <> t <> "; Reason: " <> T.pack (show x)
...
-- |
--
-- >>> Csv.toField $ addressFromTextUnsafe "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5"
-- "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5"
--
instance Csv.ToField GYAddress where
    toField = BSE.encode BSE.char8 . addressToText
...
instance Csv.FromField GYAddress where
    parseField = either (fail . T.unpack) (return . addressFromBech32) . Web.parseUrlPiece . BSE.decode BSE.char8

-- |
--
-- >>> Csv.encodeWith (Csv.defaultEncodeOptions {Csv.encUseCrLf = False}) [addressFromTextUnsafe "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5"]
-- "addr_test1qrsuhwqdhz0zjgnf46unas27h93amfghddnff8lpc2n28rgmjv8f77ka0zshfgssqr5cnl64zdnde5f8q2xt923e7ctqu49mg5\n"
--
...
instance IsString GYAddressBech32 where
    fromString = fromRight (error "invalid address") . Web.parseUrlPiece . T.pack

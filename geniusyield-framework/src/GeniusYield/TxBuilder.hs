...
import           GeniusYield.TxBuilder.Contract           as X
import           GeniusYield.TxBuilder.EmulatorTraceQuery as X
...
getCollateral addr minCollateral = do
    ...
    case xs of
        [] -> throwError $ fromString $ printf
                "no suitable ada-only UTxO with at least %d lovelace found at %s"
                minCollateral
                addr
        ys -> return $ minimumBy (compare `on` snd) ys
